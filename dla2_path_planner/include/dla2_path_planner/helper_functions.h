...
{
public:
  PathLengthOptimizationObjectiveZPenalized(const ompl::base::SpaceInformationPtr &si) : ompl::base::OptimizationObjective(si)
  {
...
ob::OptimizationObjectivePtr getBalancedObjective1(const ob::SpaceInformationPtr &si)
{
  std::cout << "HELPER - getBalancedObjective1" << std::endl;
  //auto lengthObj(std::make_shared<ob::PathLengthOptimizationObjective>(si));
  auto lengthObj(std::make_shared<PathLengthOptimizationObjectiveZPenalized>(si));
...
bool argParse(int argc, char **argv, double *runTimePtr, optimalPlanner *plannerPtr, planningObjective *objectivePtr, std::string *outputFilePtr)
{
  ...
  desc.add_options()("help,h", "produce help message")("runtime,t", bpo::value<double>()->default_value(1.0), "(Optional) Specify the runtime in seconds. Defaults to 1 and must be greater than 0.")("planner,p", bpo::value<std::string>()->default_value("RRTstar"), "(Optional) Specify the optimal planner to use, defaults to RRTstar if not given. Valid options are BFMTstar, BITstar, CForest, FMTstar, InformedRRTstar, PRMstar, RRTstar, and SORRTstar.") //Alphabetical order
      ...
      ("file,f", bpo::value<std::string>()->default_value(""), "(Optional) Specify an output path for the found solution path.")("info,i", bpo::value<unsigned int>()->default_value(0u), "(Optional) Set the OMPL log level. 0 for WARN, 1 for INFO, 2 for DEBUG. Defaults to WARN.");
